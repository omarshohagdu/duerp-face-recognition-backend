...
-- ---------------------------------------------------------------------
-- 6e · Role administration — the API behind the Access Roles screen
--
-- Generated from sql/007_access_admin.sql. Backs `POST /ext-api/access/*`
-- (src/routes/access_admin.rs), the screen that edits everything above.
--
-- This exists because the ERP's own `/access-roles` writes `ictcell` and no
-- longer reaches the copy this service enforces — docs/access_control.md §2.1
-- option (c).
--
-- THE GUARDS ARE HERE, NOT IN THE HANDLER, so they hold for a DBA at a psql
-- prompt too: `admin.roles.manage` cannot be stripped from `admin`, the last
-- active admin cannot be demoted, a role with members cannot be deleted, and
-- system roles keep the keys duerp-api joins on. Each one exists to stop an
-- administrator locking every administrator out.
--
-- NOT COPIED FROM THE MIGRATION: the endpoint rules and allow-list rows for
-- those seven paths. Without them the API is a 403 before it is reached —
-- apply sql/007_access_admin.sql.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 6e2 · Read: roles, with their permissions and how many people hold them
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 6e3 · Read: the permission vocabulary, for the tick-boxes
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 6e4 · Write: create or update a role, and replace its permission set
--
...
-- ---------------------------------------------------------------------
-- 6e6 · Read: the accounts, and what each one may do
--
...
-- ---------------------------------------------------------------------
-- 6e7 · Write: put a person in a role, or take them out of one
--
...
-- ---------------------------------------------------------------------
-- 6e8 · Write: one person's exception to their role
--
...
-- ---------------------------------------------------------------------
-- 6f · Admin-managed runtime settings — the face-verification toggle
--
-- Generated from sql/008_system_settings.sql. Backs
-- GET|PUT /admin-api/settings/nfc-face-verify (src/routes/settings.rs), and is
-- read on the card-save path through the cache in `src/utils/settings.rs`.
--
-- NOT COPIED FROM THE MIGRATION: the seeded rows, the `admin.settings.manage`
-- permission, and the endpoint / allow-list rows. Without the seed the reader
-- falls back to `NFC_FACE_VERIFY_URL` in the environment — which is the
-- pre-settings behaviour and perfectly safe; without the permission row the
-- API answers 403. Apply sql/008_system_settings.sql for a working switch.
--
-- THE SEED IS 'OFF', AND THAT DOES NOT DISABLE ANYTHING. The reader only obeys
-- this table once an admin has written to it — `updated_by IS NOT NULL` is the
-- flag — precisely so applying a migration cannot silently turn the face gate
-- off on a deployment that had it working through `.env`.
-- ---------------------------------------------------------------------

-- ---------------------------------------------------------------------
-- 6f1 · The settings themselves
--
...
-- ---------------------------------------------------------------------
-- 6f2 · Every change, with both sides
--
...
-- ---------------------------------------------------------------------
-- 6f4 · What counts as an acceptable endpoint
--
